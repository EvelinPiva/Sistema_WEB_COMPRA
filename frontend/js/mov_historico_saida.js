...
gerar_grafico.addEventListener('click', () => {
    ...
    .then(dados => {
        ...
        for (let i = 0; i < dados.length; i++) {
            let estoque = parseInt(dados[i].qtdeEstoque || 0)
            if (estoque < 1) {
        ...
        if (dadosFiltrados.length === 0) {
            resposta.innerHTML = 'Nenhum produto crítico (estoque < 1) detectado para gerar o gráfico!'
            if (meuGrafico !== null) meuGrafico.destroy()
            return
        }

        resposta.innerHTML = `Alerta! Exibindo ${dadosFiltrados.length} itens com estoque crítico (< 1).`
        ...
        for (let i = 0; i < dadosFiltrados.length; i++) {
            ...
            estoques.push(parseInt(dadosFiltrados[i].qtdeEstoque || 0))
        }
